...
function noteHandler(body = {}) {
  const command = Util.parseArgv(body?.object_attributes?.note);
  if (!command || command.nameSpace !== `/${process.env.COMMAND_PREFIX}`) {
    logger.warn(`无指令，或命名空间不正确: ${command?.nameSpace}`);
    return;
  }
  if (body?.merge_request?.state !== "merged") {
    logger.warn(`未合并，不能执行指令`);
    discussionHandler.replyDiscussion(body, `未merge，不能执行指令`);
    return;
  }
  // 其他action的扩展位置
...
function tag(body, command) {
  // 目标分支是否为release分支
  if (!Util.checkCanTagBranch(body?.merge_request?.target_branch)) {
    logger.warn("可打tag分支：release/*, custom/*");
    discussionHandler.replyDiscussion(body, `可打tag分支：release/*, custom/*`);
    return;
  }
  const {
  ...
  const tagDes =
    command?.description || command?.d || targetBranch.split("/")[1];
  ...
  if (!Util.checkTagName(tagName)) {
    discussionHandler.replyDiscussion(
      body,
  ...
  pipelineApi
    ...
    .then(
      (res) => {
        ...
        let discussionData = {
          project: _.cloneDeep(body.project),
          merge_request: _.cloneDeep(body.merge_request),
          object_attributes: _.cloneDeep(body.object_attributes),
          type: "reply",
          tag: tagName,
          isProcess: true,
      ...
      (e) => {
        ...
        logger.warn(`${userName} 在 ${body?.project?.name} 打tag的job触发失败`);
      }
    )
    .catch((e) => {
      logger.error(e);
